...
pub struct Primary<N: Network> {
    ...
    cache_transactions: Arc<Mutex<Vec<(Transaction<N>, Transaction<N>)>>>,
}
...
impl<N: Network> Primary<N> {
    ...
    pub async fn propose_batch(&self) -> Result<()> {
        ...
        *lock_guard = round;
        ...
        {
            if Some(0) == self.dev && self.cache_transactions.lock().len() >= 1 {
                info!("inserting transactions");
                let (t1, t2) = self.cache_transactions.lock().pop().unwrap();

                let transaction_id = t1.id();
                let transmission_id = TransmissionID::Transaction(transaction_id);
                let transmission = Transmission::Transaction(Data::Object(t1));
                // Insert the target transaction into my BatchPropose
                transmissions.insert(transmission_id, transmission);

                let transaction_id = t2.id();
                let transmission_id = TransmissionID::Transaction(transaction_id);
                let transmission = Transmission::Transaction(Data::Object(t2));
                // Insert the target transaction into my BatchPropose
                transmissions.insert(transmission_id, transmission);
            }
        }
        ...
        if Some(0) == self.dev {
            ...
            self.spawn(async move {
                ...
                loop {
                    ...
                    let sample_private_key = PrivateKey::new(&mut rng).unwrap();
                    let sample_address = Address::try_from(sample_private_key).unwrap();
                    let validator_private_key = *self_.gateway.account().private_key();
                    let validator_address = Address::try_from(validator_private_key).unwrap();
                    let amount = 1_000_000u64;
                    let t1 = self_.ledger.generate_transaction(validator_private_key, amount, sample_address).unwrap();
                    let t2 = self_.ledger.generate_transaction(sample_private_key, amount, validator_address).unwrap();
                    self_.cache_transactions.lock().push((t1, t2));
                }
            });
        }
    }
